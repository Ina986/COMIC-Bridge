...
export function ReplacePairingModal({ onExecute, onRescan }: Props) {
  ...
  const progress = useReplaceStore((s) => s.progress);
  const totalPairs = useReplaceStore((s) => s.totalPairs);
  const currentPair = useReplaceStore((s) => s.currentPair);
  ...
  const getModeLabel = () => {
    switch (settings.mode) {
      case "text":
        return settings.textSettings.subMode === "textLayers"
          ? "テキストレイヤー差替え"
          : `グループ差替え「${settings.textSettings.groupName}」`;
      case "image":
        return "画像差替え";
      case "batch":
        return "白消し・棒消し同時処理";
            ...
            <div className="bg-bg-tertiary rounded-xl p-4">
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  ...
                  <p className="text-text-primary font-medium">{getModeLabel()}</p>
                </div>
                <div>
                  <span className="text-text-muted text-xs">方向</span>
                  <p className="text-text-primary font-medium">{getDirectionLabel()}</p>
                </div>
                <div>

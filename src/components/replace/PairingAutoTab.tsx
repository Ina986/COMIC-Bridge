...
export function PairingAutoTab({ isReversed, onRescan }: Props) {
  const pairingJobs = useReplaceStore((s) => s.pairingJobs);
  const excludedPairIndices = useReplaceStore((s) => s.excludedPairIndices);
  const toggleExcludedPair = useReplaceStore((s) => s.toggleExcludedPair);
  const scannedFileGroups = useReplaceStore((s) => s.scannedFileGroups);
  const updatePairFile = useReplaceStore((s) => s.updatePairFile);
  const addAutoPair = useReplaceStore((s) => s.addAutoPair);
  const removeAutoPair = useReplaceStore((s) => s.removeAutoPair);
  ...
  // ペア済みファイルマップ: file → pairedFileName（ドロップダウンの表示用）
  ...
  return (
    <div className="space-y-3">
      {/* Pairing Mode Selector - Segmented Pills */}
      <div className="space-y-2">
            ...
            {PAIRING_MODES.map((mode) => (
              <button
                ...
                className={`flex-1 px-2 py-1.5 text-[11px] rounded-md transition-all whitespace-nowrap disabled:opacity-50 ${
                  settings.pairingSettings.mode === mode.value
        ...
        {/* Link Character Input (manual mode only) */}
        ...
        <div className="rounded-xl border border-border overflow-hidden">
          ...
          {showUnmatched && (
            <div className="px-3 pb-3 space-y-2">
              ...
              {/* Two-column unmatched file lists */}
              <div className="grid grid-cols-2 gap-2">
                {/* Left column */}
                <div>
                  <div className="text-[10px] text-text-muted font-medium mb-1 px-1">
                    {isReversed ? "画像データ" : "植字データ"} ({unmatchedLeft.length})
                  </div>
                ...
                <div>
                  <div className="text-[10px] text-text-muted font-medium mb-1 px-1">
                    {isReversed ? "植字データ" : "画像データ"} ({unmatchedRight.length})
                  </div>
                  <div className="border border-border/50 rounded-lg overflow-hidden">
                    <div className="max-h-[150px] overflow-y-auto p-1 space-y-0.5">

                  ...
                  return (
                    <button
                      ...
                      className={`inline-flex items-center gap-1 px-2 py-1 rounded-md border text-[10px] transition-all ${
                        onFilterFont ? "cursor-pointer hover:brightness-125 hover:scale-[1.03] active:scale-[0.98]" : ""
                      }`}
                      style={{
                        backgroundColor: `${color}10`,
                        borderColor: `${color}30`,
                      }}
                      title={onFilterFont ? `ビューアーで ${fontInfo.getFontLabel(font)} を絞り込み` : (missing ? `${font} (未インストール)` : font)}
                    ...
                    >
                      ...
                      {onFilterFont && (
                        <svg className="w-2.5 h-2.5 ml-0.5 opacity-40" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <>
                  {antiAliasStats.entries.map(([aa, count]) => {
                    ...
                    return (
                      ...
                      >
                        <span className={`font-medium ${sharp ? "text-emerald-400" : "text-red-400"}`}>
                          {getAALabel(aa)}
                        </span>
                        <span className="text-text-muted">({count})</span>
                      </span>
...
export function TextLayerRow({ entry, fontInfo, useActualFont = false, highlightFont, isSelected = false, onSelect }: { entry: TextLayerEntry; fontInfo: FontHelpers; useActualFont?: boolean; highlightFont?: string | null; isSelected?: boolean; onSelect?: () => void }) {

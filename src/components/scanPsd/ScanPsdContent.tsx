...
export function ScanPsdContent() {
  ...
  const handleAddFolder = async () => {
    ...
    if (selected && typeof selected === "string") {
      ...
      // 既に追加済みの最大巻数の次を自動設定
      const maxVol = folders.length > 0
        ? Math.max(...folders.map((f) => f.volume))
        : startVolume - 1;
        ...
        <div className="max-w-lg mx-auto space-y-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-text-muted">{scanData.processedFiles ?? 0} ファイルを処理</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <StatCard
              label="ストロークサイズ"
              value={scanData.strokeStats?.sizes?.length ?? 0}
            />
          </div>
      ...
      <div className="h-full flex items-center justify-center">
        <div className="max-w-md w-full space-y-6 px-6">
          ...
          <div className="flex items-center gap-2 justify-center">
            ...
            >
              {Array.from({ length: 50 }, (_, i) => i + 1).map((v) => (
                <option key={v} value={v}>{v}巻</option>
              ))}
            </select>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            >
              <svg className="w-4 h-4 mx-auto mb-1 opacity-60" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
              </svg>
              フォルダを個別追加
            </button>

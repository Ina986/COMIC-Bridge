    ...
    <div className="grid grid-cols-2 gap-4 items-start">
      <div className="space-y-4 min-w-0">
        <div className="flex items-center justify-between">
          <h3 className="text-xs font-black text-text-primary">フォントサイズ</h3>
          <span className="text-[10px] font-bold text-text-secondary">
            {sizeStats.sizes?.length ?? 0} サイズ
          </span>
        </div>
        {/* ベースサイズ — ヒーロー表示 */}
        <div
          ...
          {remaining.length > 0 && (
            <div className="mt-2">
              <button
                ...
                className="w-full text-[10px] font-bold text-transparent bg-accent/10 border border-accent/20 rounded-lg py-2 hover:bg-accent/15 hover:border-accent/35 transition-colors"
              >
                <span className="text-accent">その他サイズ ({remaining.length})</span>
        ...
        {sizeStats.excludeRange && (
          <div className="flex items-center gap-2 px-3 py-2.5 bg-manga-lavender/30 rounded-xl border border-accent-secondary/15">
            ...
            <div>
              <span className="text-[10px] font-bold text-accent-secondary block">
                ルビ除外範囲
              </span>
      ...
      <div className="space-y-4 min-w-0">
        <div className="flex items-center justify-between">
          <h3 className="text-xs font-black text-text-primary">白フチサイズ</h3>
          <span className="text-[10px] font-bold text-text-secondary">
            {strokeSizes.length} サイズ
          </span>
        </div>
      ...
      {remainingModalOpen && (
        <div
          ...
          onMouseDown={(e) => {
            if (e.target === e.currentTarget) setRemainingModalOpen(false);
          }}
        >
          <div
            className="w-[min(900px,calc(100vw-64px))] max-h-[72vh] overflow-hidden rounded-xl border border-border/70 bg-white shadow-xl"
            onMouseDown={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between border-b border-border-light px-4 py-3">
              <div>
                <h3 className="text-sm font-black text-text-primary">その他サイズ</h3>
              ...
              <button
                onClick={() => setRemainingModalOpen(false)}
              ...
              <div className="grid grid-cols-6 gap-2">
                {remaining.map((s) => (
                  <button
                    key={s.size}
                    onClick={() => {
                      handleTop10Click(s.size);
                      setRemainingModalOpen(false);
                    }}
                    className="rounded-lg border border-border/50 bg-bg-tertiary/50 px-2 py-2 text-left hover:border-accent/40 hover:bg-accent/5"
                  >
                    <span className="block text-xs font-black text-text-primary">{s.size}pt</span>
                    <span className="mt-1 block text-[9px] font-semibold text-text-secondary">
                      {s.count} 回
                    </span>
                  </button>
                ))}

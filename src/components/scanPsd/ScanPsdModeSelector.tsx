    ...
    <div className="h-full flex items-center justify-center bg-bg-primary">
      <div className="text-center space-y-8">
        ...
        <div className="space-y-2">
          ...
          <h2 className="text-xl font-bold text-text-primary">Scan PSD</h2>
          <p className="text-sm text-text-muted">フォントプリセット管理ツール</p>
        </div>
        ...
        <div className="flex gap-4">
          {MODES.map((mode) => (
            <button
              ...
              className="group w-56 bg-bg-secondary border border-border rounded-2xl p-6 text-left
                hover:border-accent/50 hover:shadow-md transition-all duration-200 hover:-translate-y-0.5"
            >
              <div className="w-14 h-14 rounded-xl bg-bg-tertiary flex items-center justify-center text-text-muted
                group-hover:text-accent group-hover:bg-accent/10 transition-colors mb-4">
                {mode.icon}
              </div>
              <h3 className="text-sm font-bold text-text-primary mb-1">{mode.label}</h3>

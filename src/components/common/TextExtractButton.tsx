...
export function TextExtractButton({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <>
      ...
      {result && (
        <div
          className={`w-80 bg-white rounded-2xl shadow-elevated border-2 overflow-hidden ${
          ...
          {result.success && result.filePath ? (
            <div className="px-4 py-3 space-y-2">
              ...
              <button
                ...
                className="w-full px-3 py-1.5 text-xs text-text-muted hover:text-text-primary hover:bg-bg-tertiary rounded-lg transition-colors"
              >
                閉じる
              </button>
            </div>

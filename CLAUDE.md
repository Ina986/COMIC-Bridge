...
- 仕様チェックの案内バナーを閉じられるようにし、ツールバーのiボタンから再表示できるようにした。OK/NG表示は「〇」「×」のみの表示に整理。
- レイヤー差替えの画像差替えで、特定名レイヤー名を複数指定できるようにした。フォントサイズ丸め設定の文言は「小数点以下を切り捨て」に変更。
- 写植関連の文言を「校正データ確認」「テキスト仕様確認」に変更し、テキスト仕様確認のファイル情報バーを右サイドバー上部へ移動。DTPビューアーとテキスト仕様確認の右サイドバー幅を `480px` に統一し、テキスト仕様確認のテキスト抽出ボタン/表示切替ボタンを削除。
- Scan PSDのJSON編集では左サイドバーを非表示化。新規作成ではスキャン結果が出るまで「フォント種類」以降のタブをグレーアウトし、戻るボタンは上部バーへ集約して枠線付きの矢印ボタンにした。
...
- **レイヤー順序の保持（v1.9.19〜・順序反転バグ修正）**: `replace_layers.jsx` の複製ループは「上→下順で収集 → `PLACEATBEGINNING`（最前面）で積む」構造のため、正順 `for` だと後発レイヤーが前に来て**順序が完全反転**していた（症状: 一番上のレイヤーが最下部へ）。修正方針:
...
- **ぼかし対象フォールバック**: テキストがないPSD（1レイヤーのフラットファイル含む）でも必ずぼかしが適用されるよう `pickBlurFallbackLayer()` を実装。背景SOパイプラインが `backgroundLayer` を生成しなかった場合、トップレベルの非テキストレイヤーを選び（LayerSetなら `merge()` してラスタ化）ぼかし対象にする。テキスト/textGroup/`LayerKind.TEXT` は除外
...
- **未登録フォント**（v1.9.17〜モーダル化）: ツールバーの「未登録(N)」ボタンでモーダル展開。scanData.fontsに存在するがpresetSetsに未登録のフォント一覧。個別追加 / 一括追加ボタン。0件時はdisabled
- **未インストール一覧**（v1.9.17〜モーダル化）: ツールバーの「未インストール(N)」ボタンでモーダル展開。共有フォルダから探すボタン（FontBrowserDialog起動）付き。0件時はdisabled
- **フィルタ機能**: トグルチップで絞り込み。カテゴリあり/なし（排他）、インストール済/未インストール（排他）。異なるペア間はAND
- **ソート機能**: ドロップダウンで切替。デフォルト（カテゴリ順+未インストール最下位）/ 名前順 / カテゴリ順 / 出現数順 / インストール順
- **2列カードレイアウト**（v1.9.17〜）: フォント一覧は `grid grid-cols-2 gap-1.5` のカード形式。各カードはインストール状態アイコン + カテゴリバッジ + 表示名 + PostScript名 + ホバー時に編集/削除ボタン。インライン編集時のみ `col-span-2` で全幅展開（旧テーブル + `gridCols` 変数は廃止）
...
**FontSizesTab（フォントサイズタブ）:**
- v1.9.17 で全mojibake（CP932→UTF-8文字化け: `繝輔か繝ｳ繝医し繧､繧ｺ` 等）を正しい日本語に復元。1箇所制御文字 U+0080 が混入していたためバイト単位置換で修正
...
- **完了トースト/ProGen移行UI**: 不透明な白背景+色付き2px枠（success=緑/error=赤）+色帯付きヘッダー（チェック/×アイコン+太字メッセージ）。成功時は「ProGen で開きますか？」プロンプトと**主アクションのソリッドカラーボタン**（`bg-accent-secondary` パープル+白文字+矢印アイコン+shadow-md）で `launch_progen` を呼び出す。閉じるボタンは控えめなテキストリンクに格下げ。半透明（`bg-success/10`等）による視認性低下を回避
